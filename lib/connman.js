...
var ConnMan = module.exports = function(enableAgent) {
  ...
  self.systemBus = enableAgent ? null : DBus.getBus('system');
  self.manager = null;
  self.technologies = {};
...
//    }
    ...
    self.getTechnologies(function() {
      ...
      if(self.enableAgent) {
        ...
        iface.RegisterAgent(self.Agent.path, {timeout: 10000}, function (err, result) {
            if (callback) {
                if (err) callback(err);
  ...
  // Get the bus from the Agent's serice (workaround)
  if(self.enableAgent) {
    // Initializing agent
    self.Agent = new Agent();
    self.Agent.init(function() {
        self.systemBus = self.Agent.bus;
        runInit();
    });
  } else {
    runInit();
...
ConnMan.prototype.getProperties = function(callback) {
  ...
  mgr.GetProperties({timeout: DEFAULT_TIMEOUT}, function (err, result) {
      if (err) {
          callback(err);
      } else {
...
ConnMan.prototype.setProperty = function(prop, value, callback) {
  var self = this;
  var mgr = self.manager;
  mgr.SetProperty(prop, value, {timeout: DEFAULT_TIMEOUT}, function (err, result) {
      if (err) {
        if (callback) {
          callback(err);
        }
...
ConnMan.prototype.getServices = function() {
  ...
  if (!callback) return;
  mgr.GetServices({timeout: DEFAULT_TIMEOUT}, function (err, services) {
      if (err) {
        callback(err);
      } else {
...
ConnMan.prototype.getAllTechnologyInfo = function(callback) {
  ...
  mgr.GetTechnologies({timeout: DEFAULT_TIMEOUT}, function (err, technologies) {
      if (err) {
        callback(err);
      } else {
